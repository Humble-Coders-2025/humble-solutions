  ...
  return (
    <nav className="w-full flex items-center justify-between bg-gray-700 rounded-full px-4 py-1 shadow-lg mx-auto max-w-6xl relative opacity-90">
      ...
      <div className="flex items-center space-x-2">
        <img src="/photos/logo.png" alt="Logo" className="w-30  ml-4" />
      </div>
      {/* Desktop Navigation Links */}
      <ul className="hidden md:flex space-x-8 text-white font-medium">
        <li className="relative group">
          <a href="#" className="hover:text-blue-400 transition">Home
            <span className="block h-0.5 bg-blue-400 scale-x-0 group-hover:scale-x-100 transition-transform origin-left mt-1"></span>
          </a>
        </li>
        <li className="relative group">
          <a href="#" className="hover:text-blue-400 transition">Services
            <span className="block h-0.5 bg-blue-400 scale-x-0 group-hover:scale-x-100 transition-transform origin-left mt-1"></span>
          </a>
        </li>
        <li className="relative group">
          <a href="#" className="hover:text-blue-400 transition">Portfolio
            <span className="block h-0.5 bg-blue-400 scale-x-0 group-hover:scale-x-100 transition-transform origin-left mt-1"></span>
          </a>
        </li>
        <li className="relative group">
          <a href="#" className="hover:text-blue-400 transition">Blogs
            <span className="block h-0.5 bg-blue-400 scale-x-0 group-hover:scale-x-100 transition-transform origin-left mt-1"></span>
          </a>
        </li>
        <li className="relative group">
          <a href="#" className="hover:text-blue-400 transition">About Us
            <span className="block h-0.5 bg-blue-400 scale-x-0 group-hover:scale-x-100 transition-transform origin-left mt-1"></span>
          </a>
        </li>
      </ul>
      {/* Desktop Button */}
      <button className="hidden md:block bg-blue-500 hover:bg-blue-600 text-white font-semibold px-6 py-2 rounded-full shadow transition hover:cursor-pointer mr-2">Get A Quote</button>
      {/* Hamburger Icon */}
      <button
      ...
      {menuOpen && (
        <div className="absolute top-16 left-0 w-full bg-gray-700 rounded-b-2xl shadow-lg flex flex-col items-center z-50 md:hidden animate-fade-in">
          ...
          <button className="mb-4 bg-blue-500 hover:bg-blue-600 text-white font-semibold px-6 py-2 rounded-full shadow transition">Get A Quote</button>
        </div>
      )}
    </nav>
  )
